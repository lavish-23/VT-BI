...
const steps = [
  { icon: CloudUpload, label: 'Uploading file' },
  { icon: Search, label: 'Detecting file type' },
  { icon: Database, label: 'Extracting metadata' },
  { icon: Cpu, label: 'Running AI analysis' },
  { icon: Shield, label: 'Checking for manipulation' },
  { icon: Layers, label: 'Cross-modal verification' },
  { icon: FileCheck, label: 'Generating authenticity report' },
]

// Cumulative delay for each step in ms
const stepDelays = [0, 800, 1700, 2700, 3900, 5200, 6500]
const totalDuration = 7800

function ProcessingContent() {
  const router = useRouter()
  const params = useSearchParams()
  const fileName = params.get('file') ?? 'uploaded_file'
  ...
  useEffect(() => {
    ...
    steps.forEach((_, i) => {
      timers.push(
        setTimeout(() => setCurrentStep(i), stepDelays[i]),
      )
    })
    ...
    timers.push(
      setTimeout(() => {
        setDone(true)
        router.push(`/report?file=${encodeURIComponent(fileName)}&type=${encodeURIComponent(fileType)}`)
      }, totalDuration),
    )

    return () => timers.forEach(clearTimeout)
  }, [fileName, fileType, router])
    ...
    <div className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-background px-4 py-16">
      ...
      >
        ...
        <div className="mb-8 rounded-xl bg-secondary/40 px-4 py-3">
          ...
          <p className="mt-1 truncate text-sm font-semibold text-foreground">{fileName}</p>
              ...
              <motion.circle
                ...
                animate={{
                  strokeDashoffset: done
                    ? 0
                    : 2 * Math.PI * 34 * (1 - (currentStep + 1) / steps.length),
                }}
                transition={{ duration: 0.6, ease: 'easeOut' }}
                style={{ filter: 'drop-shadow(0 0 6px oklch(0.6 0.24 260))' }}
              />
            </svg>
            ...
            return (
              ...
              >
                ...
                <span
                  className={cn(
                    'text-sm font-medium',
                    completed ? 'text-success' : active ? 'text-foreground' : 'text-muted-foreground',
                  )}
                >
                  {step.label}
                </span>
        ...
        <p className="mt-6 text-center text-xs text-muted-foreground">
          This typically takes 5–10 seconds &middot; Do not close this tab
        </p>
      </motion.div>
    </div>
  )
